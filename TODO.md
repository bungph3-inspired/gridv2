...
- [ ] **(l-cont) Remaining 3 dashboard tiles** — Cashier / Add New / Mass Edit. _Five tiles closed 2026-05-19: Bet Ticker (cont. 2) — `'ticker'` subview aggregating all wagers; Transactions (cont. 3) — `'tx'` subview synthesizing BET PLACED / BET SETTLED / FREEPLAY / SETTLEMENT from existing fields; Position (cont. 4) — `'position'` subview by-sport exposure aggregation over PENDING wagers with Worst Case (`-toWin`) and Best Case (`+risk`) columns; IP Checker (cont. 5) — `'ipcheck'` subview one row per player with synthesized IP (deterministic djb2 hash of player ID) + device + browser + lastLogin + status; Mailbox (cont. 6) — `'mailbox'` subview, player-facing two-tab support widget (canned-FAQ AI chat + email-to-support form with success card). No LLM, no live agent, no contact #._ All three remaining tiles are heavier write/action surfaces (Add New = player creation form; Mass Edit = bulk-update grid; Cashier = payments/settlements action). Defer until editability semantics are settled (thread p).
...
- [x] **(n) `verify_agent.cjs` jsdom suite** — _Closed 2026-05-19 (cont. 7): 5 sections, 36 assertions, ~1.9s runtime. Covers login splash gate (with/without bs_agent), dashboard render (11 tiles + 4 KPIs + header ID), tile routing for all 6 real subviews (weekly/pending/ticker/tx/position/ipcheck), Customer Detail nav (Management → row → tab switch from PERSONAL to LIMITS), Mailbox interactions (AI chat send with bot reply assertion, email form submit → success card → reset). Skipped: logout (calls confirm() + location.reload). Required harness extension: new `createAgentWindow` factory in harness.cjs, third bundle step in run_all.cjs Stage 2 for src/agent-main.js → bundle_agent.js. Now 12 suites total, full pipeline 10.7s._
